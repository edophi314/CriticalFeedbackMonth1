...
begin
	x_dates = ptf.date 
	PL_theo = cumsum(ptf.PL)
	PL_HWM = accumulate(max, cumsum(ptf.PL))
	DD = PL_HWM - PL_theo
	plot(x_dates, [PL_theo PL_HWM DD], 
		label= ["PL_theo" "PL_HWM (High Watermark)" "DD (drawdown)"], 
		linewidth = 2)
end
...
md"""
It is important to remember and keep in mind that the model holds directional positions for hours to a maximum of 24 hours per market.
...
md"""
Once more, the table above shows *paper pl (PL)* vs *real pl (plr)*:
...
b- The 4th and 5th September plr differ from PL significantly : this is prior to the email sent to Clement Mary On September 13th 2023 regarding order generation on FULL portfolio update within a single (region, asset class). I didn't publish the orders correctly.

c- The 25th and 26th: the SP500 was traded by the paper model from the open. In real life, upon opening, the market immediately dipped down. We set an alert to buy it back on a second touch at the opening price: that never happened. This is typically the case of a threshold effect that played in our favour.
...
md"""
1) This type of trading methodology is  timely: the popularity of stock indexes as instruments of speculation for the general public, the explosion of interest on futures, ETFs and CFDs ensures both deep liquidity and the pertinence of relying on deeply embedded  psychological structures and biaises from crowd aggregated behavior.
2) The model as traded today represents the bone marrow of a technology that we are refining and completing on an on-going basis.
3) Very few people if any implement such a program, to the best of our knowledge. Our trader, with years of experience in arbitrage strategies (index vs cash, options, yiled curve...), hardly knows any European CTA positioning themselves on directional behavioral trading.
4) The capacity of the program, simply as presented here, turns out to be more than initially expected.
5) Our best guess is that the specifities of weekly intervention and the α captured here would nicely improve and diversify the SR of any other strategy.
"""

# ╔═╡ 72693956-2bf2-4fdb-baab-939bad65e9c0
html"""
<h2 style="color:lightblue;text-align:left;">5 . - Initial marketing presentation sent July 7th 2023:</h2>
"""
